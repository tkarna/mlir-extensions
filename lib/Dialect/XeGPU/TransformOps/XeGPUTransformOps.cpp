...
/// Set default attributes to xegpu.load_nd op.
void setLoadOpAttrs(Builder builder, xegpu::LoadNdOp loadOp,
                    bool packed = false) {
  auto ctx = builder.getContext();
  auto readCacheHint =
      xegpu::CachePolicyAttr::get(ctx, xegpu::CachePolicy::CACHED).getValue();
  loadOp.setL1Hint(readCacheHint);
  loadOp.setL2Hint(readCacheHint);
  loadOp.setL3Hint(readCacheHint);
  loadOp.setPacked(packed);
}

/// Set default attributes to xegpu.store_nd op.
void setStoreOpAttrs(Builder builder, xegpu::StoreNdOp storeOp) {
  auto ctx = builder.getContext();
  auto writeCacheHint =
      xegpu::CachePolicyAttr::get(ctx, xegpu::CachePolicy::WRITE_BACK)
          .getValue();
  storeOp.setL1Hint(writeCacheHint);
  storeOp.setL2Hint(writeCacheHint);
  storeOp.setL3Hint(writeCacheHint);
}
...
/// Folds defining memref.SubviewOp into the xegpu.CreateNDDescOp if
/// the subview is unit stride and in the same region.
...
/// Get defining op of the given type
...
FailureOr<scf::ForOp> hoistLoadStoreOps(transform::TransformRewriter &rewriter,
  ...
  for (auto &op : loopOp.getBody()->getOperations()) {
    ...
    if (!tileToInitValueMap.contains(tile)) {
      // case 1: tile has not been seen yet
      // set default load attributes
      setLoadOpAttrs(rewriter, loadOp);
  ...
  // rewrite loop with new init/yield values
  SmallVector<Value> initValues, yieldValues;
  for (auto &tile : descTiles) {
    auto initValue = tileToInitValueMap[tile];
    auto yieldValue = tileToYieldValueMap[tile];
    initValues.push_back(initValue);
    yieldValues.push_back(yieldValue);
  }
  auto nbYields = loopOp.getNumResults();
  NewYieldValuesFn yieldFn = [&](OpBuilder &b, Location loc,
                                 llvm::ArrayRef<BlockArgument> newBBArgs) {
    return yieldValues;
  };
  ...
  for (auto [i, tile] : llvm::enumerate(descTiles)) {
    auto yieldIndex = nbYields + i;
    ...
    auto newStoreOp = rewriter.create<xegpu::StoreNdOp>(
        newLoopOp.getLoc(), TypeRange{}, ValueRange{storeValue, tile});
    setStoreOpAttrs(rewriter, newStoreOp);
  }
...
/// Fold redundant load ops in the loop body if read only semantics.
  ...
  for (auto &[tile, loadOps] : tileToLoadOps) {
    ...
    for (auto &loadOp : loadOps) {
      if (firstOp) {
        ...
        firstOp = false;
        // set attrs
        setLoadOpAttrs(rewriter, cast<xegpu::LoadNdOp>(loadOp));
        continue;
      }
...
/// Cast the accumulator type of dpas op to f32 if necessary.
...
/// Insert a gpu thread sync op at the beginning of the loop body.
LogicalResult insertThreadSync(transform::TransformRewriter &rewriter,
                               scf::ForOp loopOp, int64_t syncSplit,
                               int64_t maxSyncStep) {
  ...
  int syncStep = std::min(kDimSize / syncSplit, maxSyncStep);
...
/// Return the i-th subtile of the source memref with the given tile size.
/// Upper bound is used to check that index does not exceed the number of
/// subtiles.
std::optional<Value> getIthSubtile(transform::TransformRewriter &rewriter,
  ...
  if (!isa<MemRefType>(source.getType())) {
    LLVM_DEBUG(llvm::dbgs() << "Source is not a memref.\n");
    return std::nullopt;
  }
...
DiagnosedSilenceableFailure transform::XeGPUHoistLoadStoreOp::applyToOne(
  ...
  loopOp = *newLoopOp;
  foldRedundantLoadOps(rewriter, loopOp);
  results.push_back(loopOp.getOperation());
  return DiagnosedSilenceableFailure::success();
}
...
DiagnosedSilenceableFailure transform::XeGPUSetLoadTileOp::applyToOne(
  ...
  for (auto &descOp : descOps) {
    ...
    if (llvm::is_contained(createdLoadTiles, loadTileKey)) {
      ...
      // create new load op
      auto loadOp = rewriter.create<xegpu::LoadNdOp>(loc, loadVecType,
                                                     newDescOp.getResult());
      // set attributes for the load op
      setLoadOpAttrs(rewriter, loadOp, useVnni);
      // add to cache
    ...
    Value slicedVec;
    if (useVnni) {
      assert(targetNestedOffsets[1] == 0 &&
             "2d layout not supported for VNNI loads");
      ...
      slicedVec = rewriter
                      .create<vector::ExtractStridedSliceOp>(
                          loc, loadedTileValue,
                          /*offsets=*/targetNestedOffsets,
                          /*sizes=*/targetTileShape,
                          /*strides=*/ArrayRef<int64_t>{1, 1})
                      .getResult();
    }
    // replace old load op with the new tile
    rewriter.replaceOp(oldLoadOp, slicedVec);
  }
...
DiagnosedSilenceableFailure transform::XeGPUInsertThreadSyncOp::applyToOne(
    transform::TransformRewriter &rewriter, Operation *target,
    transform::ApplyToEachResultList &results,
    transform::TransformState &state) {

  auto syncSplit = getSyncSplit();
  auto maxStep = getMaxStep();
  auto forOp = dyn_cast<scf::ForOp>(target);
  if (!forOp) {
    auto diag = emitSilenceableFailure(getLoc())
                << "Expected a scf.for op, but got: " << target->getName();
  ...
  if (failed(insertThreadSync(rewriter, forOp, syncSplit, maxStep))) {
    auto diag = emitSilenceableFailure(getLoc())
                << "Failed to add thread sync op";
    diag.attachNote(forOp.getLoc()) << "loop op";
    return diag;
  }
  return DiagnosedSilenceableFailure::success();
}

void transform::XeGPUInsertThreadSyncOp::getEffects(
    ::llvm::SmallVectorImpl<MemoryEffects::EffectInstance> &effects) {
  onlyReadsHandle(getLoopMutable(), effects);
  modifiesPayload(effects);
}
