...
// Ported from include/gc/Transforms/Utils/ValueUtils.h
...
// Ported from graph-compiler/lib/gc/Transforms/Utils/ValueUtils.cpp
...
// ported from graph-compiler/lib/gc/Transforms/Utils/MatcherUtils.cpp
...
static bool isTppOp(linalg::LinalgOp linalgOp) {
  // clang-format off
  auto tppMatcher =
    structured_match::StructuredOpMatcher::make<linalg::LinalgOp>()
...
static LogicalResult isValidMemrefOperand(linalg::LinalgOp linalgOp,
  ...
  if (type.getShape().size() > maxDims &&
      !canSqueezeDims(type.getShape(), maxDims)) {
    return rewriter.notifyMatchFailure(
        linalgOp, "Too high dimensionality for XeGPU operations");
  }

  auto strides = getStaticStrides(operand);
...
static std::optional<Value> lowerGenericOp(linalg::GenericOp genericOp,
  ...
  if (isTwoDReluOp(genericOp, /*operands=*/nullptr)) {
  ...
  if (isTwoDAddOp(genericOp, /*operands=*/nullptr)) {
...
static SmallVector<Value> createScatterDescriptorTiles(
  ...
  auto offsetPerLoad = createTypedVector<int64_t>(
  ...
  for (int i = 0; i < numColTiles; i++) {
    auto offsetsShift = createTypedVector<int64_t>(
...
static SmallVector<Value> createSLMDescTiles(PatternRewriter &rewriter,
  ...
  // offset.
  computeSubviewOffsets(rewriter, loc, src, offsets, rootMemref);
  auto rootStridesFold = getMemrefStrides(rewriter, loc, rootMemref);
  auto rootStrides =
  ...
  auto memrefStridesFold = getMemrefStrides(rewriter, loc, src);
  ...
  src = flattenMemref(rewriter, loc, rootMemref);
...
static Value findAndReplaceBroadcast(linalg::LinalgOp linalgOp,
  ...
  while (auto defOp = operandParent.getDefiningOp()) {
    for (auto x : defOp->getUsers()) {
      ...
      if (broadcastOp) {
        static_cast<void>(rewriter.notifyMatchFailure(
            broadcastOp, "Only one broadcast operation is allowed"));
        return operand;
      }
      ...
      if (!onlyUsedByOp(broadcastRes, ops)) {
        static_cast<void>(rewriter.notifyMatchFailure(
            broadcastOp,
            "Broadcast result is used by more than one operation"));
        return operand;
      }
      break;
    }
  ...
  if (inpShape.size() < 2) {
    static_cast<void>(rewriter.notifyMatchFailure(
        broadcastOp, "Only nD broadcast is supported"));
    return operand;
  }

  if (!canSqueezeDims(inpShape) || !canSqueezeDims(outShape)) {
    static_cast<void>(rewriter.notifyMatchFailure(
        broadcastOp, "Can't squeeze broadcast operands to 2D"));
    return operand;
  }

  auto res = reduceMemrefDims(rewriter, broadcastOp.getLoc(), brInp);
  if (failed(res))
    return operand;
...
static LogicalResult createDPASKernel(linalg::LinalgOp linalgOp,
  ...
  auto dpasResType = VectorType::get(dpasShapeC, rewriter.getF32Type());
  ...
  if (failed(blockDims)) {
    blockDims = getStaticBlockSizes(linalgOp->getParentOfType<gpu::LaunchOp>());
  }
  ...
  VnniConfig vnniConfB{vnniFactor, 0};
...
struct ConvertGemmLikeToXeGPU : public OpRewritePattern<LinalgOpTy> {
  ...
  LogicalResult matchAndRewrite(LinalgOpTy gemmLikeOp,
    ...
    if (failed(maybeSqueezeDims(rewriter, gemmLikeOp))) {
      return rewriter.notifyMatchFailure(
...
struct ConvertNamedEltwiseToXeGPU : public OpRewritePattern<LinalgOpTy> {
  ...
  LogicalResult matchAndRewrite(LinalgOpTy eltwiseOp,
    ...
    if (failed(maybeSqueezeDims(rewriter, eltwiseOp))) {
      return rewriter.notifyMatchFailure(
          eltwiseOp,
...
LogicalResult createMemoryFillKernel(linalg::LinalgOp linalgOp,
  ...
  int64_t maxSizeSIMD = hasSharedMemSpace(output) ? maxSLMTileSize : 256;
...
struct ConvertMemoryFillToXeGPU : public OpRewritePattern<LinalgOpTy> {
  ...
  LogicalResult matchAndRewrite(LinalgOpTy linalgOp,
    ...
    if (failed(maybeSqueezeDims(rewriter, linalgOp))) {
      return rewriter.notifyMatchFailure(
          linalgOp,
...
struct LinalgToXeGPU : public imex::impl::LinalgToXeGPUBase<LinalgToXeGPU> {
