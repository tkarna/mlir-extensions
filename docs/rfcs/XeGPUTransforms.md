...
This proposal outlines four new operations to fill the gaps. These operators will reside in the XeGPU namespace of the transform dialect:
...
These operations are sufficient for lowering a `linalg.matmul` operation to XeGPU dialect and provide necessary transforms to obtain near peak performance on Intel GPUs (PVC, 4k matmul benchmark).
...
```mlir
        ...
        scf.for %arg7 = %c0 to %c4096 step %c32 {
          ...
          %11 = xegpu.create_nd_tdesc %subview_9[0, 0] : memref<8x16xf16, strided<[4096, 1], offset: ?>> -> !xegpu.tensor_desc<8x16xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = false>>
          xegpu.store_nd %10, %11  : vector<8x16xf16>, !xegpu.tensor_desc<8x16xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = false>>
          ...
        }
```
...
The descriptor is added in the loops `iter_args`, updating the offset by k-tile size every iteration.

Note that the loop-independent C tile ops are not affected.
...
```mlir
        ...
        %68:32 = scf.for %arg7 = %c0 to %c4096 step %c32 iter_args(..., %arg26 = %67, ...) -> (..., vector<8x16xf32>, ...) {
          %85 = arith.remui %arg7, %c1024 : index
          %86 = arith.cmpi eq, %85, %c0 : index
          scf.if %86 {
            gpu.barrier // thread barrier
          }
          %87 = xegpu.update_nd_offset %arg8, [0, %c32] : !xegpu.tensor_desc<8x16xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = false>>
          %88 = xegpu.load_nd %arg8  : !xegpu.tensor_desc<8x16xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = false>> -> vector<8x16xf16>
          %89 = xegpu.update_nd_offset %arg9, [%c32, 0] : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = false>>
          %90 = xegpu.load_nd %arg9  : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = false>> -> vector<16x16xf16>
          %91 = xegpu.dpas %88, %90, %arg26 : vector<8x16xf16>, vector<16x16xf16>, vector<8x16xf32> -> vector<8x16xf32>
          ...
          %123 = xegpu.dpas %120, %122, %91 : vector<8x16xf16>, vector<16x16xf16>, vector<8x16xf32> -> vector<8x16xf32>
          ...
          scf.yield ..., %123, ... : ..., vector<8x16xf32>, ...
        }
        ...
        %74 = arith.truncf %68#18 : vector<8x16xf32> to vector<8x16xf16> // f32 -> f16
        xegpu.store_nd %74, %10 <{l1_hint = #xegpu.cache_hint<write_back>, l2_hint = #xegpu.cache_hint<write_back>, l3_hint = #xegpu.cache_hint<write_back>}> : vector<8x16xf16>, !xegpu.tensor_desc<8x16xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = false>>
        ...
```

This is a DPAS specific pattern. Addresses transformations B, C and D:
...
```mlir
%prefetch_loop_a = transform.xegpu.insert_prefetch %sg_matmul 0 [8, 32] : (!transform.any_op) -> !transform.any_op
```

The index indicates the matmul operand tile (0 for A) and the tile size speficies the desired prefect tile size. The tile size must be compatible with the SG A tile and the number of sharing threads (resulting in a 4-way row decompostion in this case).
...
As seen above, it creates the descriptor for the thread's subtile, prefetches it, and updates the offset on every iteration. The new `%prefetch_loop_a` can be fused to the reduction loop with `transform.loop.fuse_sibling` operation.
...
```mlir
transform.xegpu.set_load_tile %loop_k2 0 [32, 32] : !transform.any_op
```
...
```mlir
        scf.for %arg7 = %c0 to %c4096 step %c32 {
          %subview_5 = memref.subview %subview_2[0, %arg7] [32, 32] [1, 1] : memref<32x4096xf16, strided<[4096, 1], offset: ?>> to memref<32x32xf16, strided<[4096, 1], offset: ?>>
          %4 = xegpu.create_nd_tdesc %subview_5[0, 0] : memref<32x32xf16, strided<[4096, 1], offset: ?>> -> !xegpu.tensor_desc<32x32xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = true>>
          %5 = xegpu.load_nd %4 <{l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<cached>, l3_hint = #xegpu.cache_hint<cached>}> : !xegpu.tensor_desc<32x32xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = true>> -> vector<32x32xf16>
          %6 = vector.shape_cast %5 : vector<32x32xf16> to vector<1024xf16>
          %7 = vector.extract_strided_slice %6 {offsets = [0], sizes = [128], strides = [1]} : vector<1024xf16> to vector<128xf16>
...
```mlir
transform.xegpu.set_load_tile %loop_k2 1 [32, 16] : !transform.any_op
```
...
```mlir
        scf.for %arg7 = %c0 to %c4096 step %c32 {
          %subview_6 = memref.subview %subview_3[%arg7, 0] [32, 64] [1, 1] : memref<4096x64xf16, strided<[4096, 1], offset: ?>> to memref<32x64xf16, strided<[4096, 1], offset: ?>>
          %6 = xegpu.create_nd_tdesc %subview_6[0, 0] : memref<32x64xf16, strided<[4096, 1], offset: ?>> -> !xegpu.tensor_desc<32x16xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = true>>
          %7 = xegpu.load_nd %6 <{l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<cached>, l3_hint = #xegpu.cache_hint<cached>, packed}> : !xegpu.tensor_desc<32x16xf16, #xegpu.block_tdesc_attr<memory_space =  global, array_length = 1 : i64, boundary_check = true>> -> vector<16x16x2xf16>
          %8 = vector.shape_cast %7 : vector<16x16x2xf16> to vector<512xf16>
          %9 = vector.extract_strided_slice %8 {offsets = [0], sizes = [256], strides = [1]} : vector<512xf16> to vector<256xf16>
          %10 = vector.shape_cast %9 : vector<256xf16> to vector<8x16x2xf16>
          ...
          %13 = xegpu.dpas %5, %10, %12 : vector<8x16xf16>, vector<8x16x2xf16>, vector<8x16xf16> -> vector<8x16xf16>
          ...
        }
```

Addresses transformations F and G.
...
module attributes {transform.with_named_sequence} {
  transform.named_sequence @__transform_main(%arg0: !transform.any_op) {
    ...
    %prefetch_loop_a = transform.xegpu.insert_prefetch $sg_matmul 0 [8, 32] : (!transform.any_op) -> !transform.any_op
    %prefetch_loop_b = transform.xegpu.insert_prefetch $sg_matmul 1 [8, 32] : (!transform.any_op) -> !transform.any_op

    // tile inner matmul op to DPAS size, change loop order to k,n,m
    %dpas_matmul, %dpas_loops:3 = transform.structured.tile_using_for $sg_matmul tile_sizes [8, 16, 16] interchange = [2, 0, 1] : (!transform.any_op) -> (!transform.any_op, !transform.any_op, !transform.any_op, !transform.any_op)

    // Vectorize linalg.matmul
    %mma = transform.structured.match ops{["linalg.matmul"]}
      in %loop_k : (!transform.any_op) -> !transform.any_op
    transform.structured.vectorize %mma : !transform.any_op

    // Patterns to convert to vector.contract, only in innermost DPAS loop
    transform.apply_patterns to %dpas_loops#2 {
      transform.apply_patterns.vector.reduction_to_contract
      // Reduce the rank of xfer ops. This transforms vector.contract to be
      // more matmul-like and to enable the lowering to outer product Ops.
    ...
    // canonicalize
    transform.apply_cse to %loop_k : !transform.any_op
    ...
    // set load tile size for DPAS A tiles
    transform.xegpu.set_load_tile %loop_k2 0 [32, 32] : !transform.any_op
    // set load tile size for DPAS B tiles
    transform.xegpu.set_load_tile %loop_k2 1 [32, 16] : !transform.any_op
...
* Load tile sizes for A and B: [32, 32], [16, 32]

## Performance

The above schedule yields 300 TFLOPS/s performance on a single PVC tile.
